...
class PlayAction(object):
    ...
    def __init__(self, say):
        self.player = Player('scotomic813@gmail.com', 'Meribel14', '380277856539da1c')
        self.say = say
    ...
    def run(self, voice_command):
        ...
        if self.player.load_playlist(playlist):
            ...
            while self.player.playing and gpio.input(23):
                time.sleep(1)
...
class PlayYouTube(object):
    ...
    def run(self, voice_command):
        ...
        logging.info("YouTube " + track)
        self.say("Searching YouTube for " + track)
...
def make_actor(say):
    ...
    actor.add_keyword(_('listen to'), PlayRadio(say,_('listen to')))
    actor.add_keyword(_('tune in to'), PlayRadio(say,_('tune in to')))
    return actor
